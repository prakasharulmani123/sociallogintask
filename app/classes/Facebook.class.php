...
class Facebook {
    ...
    public static function get_email() {
        ...
        if (isset($_GET['code'])) {
            $user_data = array();
            $token = Handling::curlHttpRequest("https://graph.facebook.com/oauth/access_token?client_id=" . $Configuration['facebook_appid'] . "&redirect_uri=" . $Configuration['facebook_redirect_url'] . $Configuration['facebook_redirect_url_slug'] . "&client_secret=" . $Configuration['facebook_appsecret'] . "&code=" . $_GET['code'] . "&scope=user_birthday,email,publish_actions");
            ...
            if (!empty($token)) {
                ...
                $user_data['user']['gender'] = $request->birthday;
        ...
        #Auth URL
        $url = "https://www.facebook.com/v2.12/dialog/oauth?client_id=" . $Configuration['facebook_appid'] . "&redirect_uri=" . $Configuration['facebook_redirect_url'] . $Configuration['facebook_redirect_url_slug'] . "&response_type=code&scope=user_birthday,email,publish_actions";
